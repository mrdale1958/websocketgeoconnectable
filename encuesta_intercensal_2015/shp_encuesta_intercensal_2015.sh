...
function reproject {
  ...
  do
    ...
    then
      ...
      echo "No territoria insular in " "$1"/$2_${files_nice[$i]}
      #exit 1
    fi
  done
...
do
   FILENAME="$URL${state_names[$counter]}/$state_number$STEM"
   # download the files from the inegi server. 
   $WGET $FILENAME -O ${states[$counter]}.zip
   # Extract the shapefiles from the zip file
   mkdir -p shps/${states[$counter]}
   unzip -L ${states[$counter]}.zip -d shps/${states[$counter]}
